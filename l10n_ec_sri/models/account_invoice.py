...
class AccountInvoice(models.Model):
    ...
    def name_search(self, name, args=None, operator="ilike", limit=100):
        ...
        if name:
            recs = self.search(
                ["|", "|", ("secuencial", "=", name), ("name", "=", name)] + args,
                limit=limit,
            )
        if not recs:
    ...
    def normalize_date(self, date, fmt="dmy"):
        if fmt == "dmy":
            # TODO JV: return datetime.strptime(date, "%Y-%m-%d").strftime("%d-%m-%Y")
            return "{}-{}-{}".format(date.day, date.month, date.year)
        

    def date_tz(self, date, tz="America/Guayaquil"):
        # TODO JV: date = datetime.strptime(date, "%Y-%m-%d")
    ...
    def get_sri_ats_lines(self):
        for inv in self:
            ...
            sustentos = inv.invoice_line_ids.mapped("tax_ids").mapped(
                "sustento_id.code"
            )
            ...
            for s in sustentos:
                ...
                for line in inv.invoice_line_ids:
                    codsustento = line.mapped("tax_ids").mapped(
                        "sustento_id.code"
                    ) or ["NA"]
    ...
    def get_sri_tax_lines(self):
        ...
        for inv in self:
            ...
            for tax_line in unique_tax_lines:
                ...
                amount = sum(
                    inv.line_ids.filtered(lambda l: tax == l.tax_id).mapped(
                        "amount"
                    )
                )
            ...
            for line in lines:
                ...
                tax_obj = line.tax_ids.compute_all(
                    price_unit,
                    currency=currency,
    ...
    def compute_taxes(self):
        ...
        if self.invoice_line_ids:
            sub_total = sum ( invoice.price_subtotal for invoice in self.invoice_line_ids)
            total = sum ( invoice.price_total for invoice in self.invoice_line_ids)
            
            impgrav_lines = self.invoice_line_ids.filtered(lambda l: "ImpGrav" in l.tax_ids.mapped("tax_group_id.name"))
            if impgrav_lines:
                baseimpgrav = sum( impgrav.price_subtotal for impgrav in impgrav_lines ) or 0
                
            nograiva_lines = self.invoice_line_ids.filtered(lambda l: "NoGraIva" in l.tax_ids.mapped("tax_group_id.name"))
            if nograiva_lines:
                basenograiva = sum (nograv.price_subtotal for nograv in nograiva_lines ) or 0

            impexe_lines = self.invoice_line_ids.filtered(lambda l: "ImpExe" in l.tax_ids.mapped("tax_group_id.name"))
            if impexe_lines:
                baseimpexe = sum(line.price_subtotal for line in impexe_lines) or 0

            imponible_lines = self.invoice_line_ids.filtered(lambda l: "Imponible" in l.tax_ids.mapped("tax_group_id.name"))
            if imponible_lines:
    ...
    def compute_sri_invoice_amounts(self):
        ...
        impgrav_lines = inv_lines.filtered(
            lambda l: "ImpGrav" in l.tax_ids.mapped("tax_group_id.name")
        )
        
        for x in inv_lines:
            _logger.warning("PPPPPPPPPPPPPPPPPPP")
            _logger.warning(x.name)
            _logger.warning(x.price_total)
        ...
        nograiva_lines = inv_lines.filtered(
            lambda l: "NoGraIva" in l.tax_ids.mapped("tax_group_id.name")
        )
        ...
        imponible_lines = inv_lines.filtered(
            lambda l: "Imponible" in l.tax_ids.mapped("tax_group_id.name")
        )
        baseimponible = sum(line.price_subtotal for line in imponible_lines) or 0
        inv_lines -= imponible_lines

        impexe_lines = inv_lines.filtered(
            lambda l: "ImpExe" in l.tax_ids.mapped("tax_group_id.name")
        )
        baseimpexe = sum(line.price_subtotal for line in impexe_lines) or 0
        ...
        montoiva = sum(
            line.amount for line in tax_lines if line.tax_group_id.name == "ImpGrav"
        )
        montoice = sum(
            line.amount for line in tax_lines if line.tax_group_id.name == "Ice"
        )
...
class SriAtsLine(models.Model):
    ...
    invoice_id = fields.Many2one(
        "account.move", ondelete="cascade", string="Invoice", required=False
    )
...
class SriTaxLine(models.Model):
    ...
    invoice_id = fields.Many2one(
        "account.move", ondelete="restrict", string="Invoice"
    )
    ...
    r_invoice_id = fields.Many2one(
        "account.move", ondelete="restrict", string="Sale invoice (for retentions)"
    )
